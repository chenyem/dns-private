...
import c from "kleur";
import { $argv, relativePath, ROOT_PATH, UPDATER_JSON_PATH } from "./utils.mjs";
...
export default async function updater() {
  ...
  console.log(
    c.yellow("*"),
    c.gray("value:"),
    c.green(relativePath(path.dirname(filename)))
  );
  console.log(
    c.yellow("*"),
    c.blue("Edit `tauri.conf.json > tauri > updater > endpoints`")
  );
  console.log(
    c.yellow("*"),
    c.gray("value:"),
    c.green(`https://${owner}.github.io/${repo}/${path.basename(filename)}`)
  );
  console.log(c.yellow("*".repeat(20)));
}
